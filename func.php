...
function request($url, $custom = null, $delcookie = false) {
    $ch = curl_init();
        ...
        if($custom != null && is_array($custom)) {
            if(isset($custom["cookiefile"])) {
            	$fp = fopen($custom["cookiefile"], "w"); fclose($fp);
                curl_setopt($ch, CURLOPT_COOKIEJAR, $custom["cookiefile"]);
                curl_setopt($ch, CURLOPT_COOKIEFILE, $custom["cookiefile"]);
            }
            if(isset($custom["httpheader"]) && is_array($custom["httpheader"])) {
                curl_setopt($ch, CURLOPT_HTTPHEADER, $custom["httpheader"]);
            }
            if(isset($custom["postdata"])) {
                $postdata = (is_array($custom["postdata"]) ? http_build_query($custom["postdata"]) : $custom["postdata"]);
                curl_setopt($ch, CURLOPT_POST, 1);
                curl_setopt($ch, CURLOPT_POSTFIELDS, $postdata);
            }
            if(isset($custom["referer"])) {
                curl_setopt($ch, CURLOPT_REFERER, $custom["referer"]);
            }
            if(isset($custom["useragent"])) {
                curl_setopt($ch, CURLOPT_USERAGENT, $custom["useragent"]);
            }
        }
    $page = curl_exec($ch);
    curl_close($ch);
    if($delcookie && isset($custom["cookiefile"]) && file_exists($custom["cookiefile"])) unlink($custom["cookiefile"]);
    return $page;
}
function getCookie($cookiefile, $cookname) {
	$source = file_get_contents($cookiefile);
	preg_match_all("#".$cookname."	(.*?)\n#i", $source, $matches);
	return $matches[1][0];
}
...
function genStr($length = 7, $withnum = false, $caps = false) {
